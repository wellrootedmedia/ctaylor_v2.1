<?php
	...
	foreach ($eximg as $eximg) {
		...
		if($imgname <> '') {
			...
			$SQL3 = "UPDATE ".$wpdb->prefix."photocrati_galleries SET gal_type = " . ((int)$_POST['gal_type']) . ", image_order = " . ((int)$_POST['image_order']) . ", image_alt = '" . $wpdb->escape(htmlspecialchars($_POST['image_alt'])) . "', image_desc = '" . $wpdb->escape(htmlspecialchars($_POST['image_desc'])) . "', ecomm_options = '" . $wpdb->escape($_POST['ecomm_options']) . "' WHERE gallery_id = '" . $wpdb->escape($_POST['gallery_id']) . "' AND image_name = '" . $wpdb->escape(htmlspecialchars($_POST['image_name'])) . "'";
			$wpdb->query($SQL3);
